...
def update_data(data, custom_kwargs, index):
    ...
    if custom_kwargs['line_labels'] is not None:
        ...
        data['hovertemplate'] = re.sub(f"={index}", f"={data['name']}", data['hovertemplate'])
    return
...
def update_layout(layout, custom_kwargs, is_animation):
    if custom_kwargs['debug']: print(layout, is_animation)
...
def update_fig(fig, custom_kwargs, inplace=True):
    if custom_kwargs['debug']: print(fig.frames==tuple())
    if not inplace:
