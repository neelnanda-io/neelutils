...
def imshow(tensor, xaxis=None, yaxis=None, animation_name='Snapshot', return_fig=False, **kwargs):
    # tensor = torch.squeeze(tensor)
    if 'x' in kwargs:
        if len(kwargs['x'])!=len(set(kwargs['x'])):
            logging.warning(f"X axis labels are not unique, this will mess up imshow: {kwargs['x']}")
    if 'y' in kwargs:
        if len(kwargs['y'])!=len(set(kwargs['y'])):
            logging.warning(f"Y axis labels are not unique, this will mess up imshow: {kwargs['y']}")
...
def scatter(x, y, hover=None, return_fig=False, yaxis="", xaxis="", **kwargs):
    fig = px.scatter(x=to_numpy(x, flat=True), y=to_numpy(y, flat=True), hover_name=hover, **kwargs)
    fig.update_layout(xaxis_title=xaxis, yaxis_title=yaxis)
    if return_fig:
        return fig
    else:
        fig.show()
def lines(lines_list, x=None, mode='lines', labels=None, xaxis='', yaxis='', title = '', log_y=False, hover=None, return_fig=False, **kwargs):
...
def animate_lines(lines_list, snapshot_index = None, snapshot='snapshot', hover=None, xaxis='x', yaxis='y', return_fig=False, **kwargs):
    ...
    print(df)
    fig = px.line(df, x=xaxis, y=yaxis, animation_frame=snapshot, range_y=[lines_list.min(), lines_list.max()], hover_name=hover,**kwargs)
    if return_fig:
        return fig
    else:
        fig.show()

def animate_multi_lines(lines_list, y_index=None, snapshot_index = None, snapshot='snapshot', hover=None, swap_y_animate=False, return_fig=False, **kwargs):
    ...
    fig = px.line(df, x='x', y=y_index, animation_frame=snapshot, range_y=[lines_list.min(), lines_list.max()], hover_name=hover, **kwargs)
    if return_fig:
        return fig
    else:
        fig.show()

def animate_scatter(lines_list, snapshot_index = None, snapshot='snapshot', hover=None, yaxis='y', xaxis='x', color=None, color_name = 'color', return_fig=False, **kwargs):
    ...
    fig = px.scatter(df, x=xaxis, y=yaxis, animation_frame=snapshot, range_x=[lines_list[:, 0].min(), lines_list[:, 0].max()], range_y=[lines_list[:, 1].min(), lines_list[:, 1].max()], hover_name=hover, color=color_name, **kwargs)
    if return_fig:
        return fig
    else:
        fig.show()
...
def add_button(fig, button, auto_pos=True):
    ...
    if 'x' not in button:
        button['x']=-0.1
    fig.update_layout(
        updatemenus=fig.layout.updatemenus+(button,)
    )
    return fig
